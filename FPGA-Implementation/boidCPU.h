 ...
 * boidCPU.h
 * 
 * This is the header file for a BoidCPU FPGA core. A BoidCPU is a processing
 * unit responsible for an area of the simulation space. Any boids that are
 * within a BoidCPU's bounds are managed and controlled by that BoidCPU. As
 * boids move around the simaulation area they move between BoidCPU areas and
 * so are transferred between BoidCPUs. 
 * 
 * This file primarily contains function definitions and #defines that are used 
 * in boidCPU.cpp to aid readability of the source code. The #defines in 
 * this file are supposed to be identical to those in boidMaster.h and boids.h. 
 * Ideally, there would be no duplication, but the tools used did not support a 
 * common file. 
 * 
...
#include "hls_math.h"               // For square root (sqrt())
